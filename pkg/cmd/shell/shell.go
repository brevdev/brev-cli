...
func runShellCommand(t *terminal.Terminal, sstore ShellStore, workspaceNameOrID string) error {
	...
	_ = writeWSConnectionEvent(sstore, sshName) // we don't care about the error here but should log with sentry
...
func writeWSConnectionEvent(store wce, name string) error {
