...
type fakeOpenEnvironmentService struct {
	...
	sequentialPorts   []*devplanev1.Port
...
type sequentialOpenRequest interface {
	GetFromPortNumber() int32
	GetToPortNumber() int32
	GetProtocol() devplanev1.PortProtocol
	GetAllowedSources() []string
}
...
func assertSequentialOpenRequest(t *testing.T, want, got sequentialOpenRequest) {
	t.Helper()
	assert.Equal(t, want.GetFromPortNumber(), got.GetFromPortNumber())
	assert.Equal(t, want.GetToPortNumber(), got.GetToPortNumber())
	assert.Equal(t, want.GetProtocol(), got.GetProtocol())
	assert.Equal(t, want.GetAllowedSources(), got.GetAllowedSources())
}
...
func (s *fakeOpenEnvironmentService) OpenSequentialPorts(
	_ context.Context,
	req *connect.Request[devplanev1.EnvironmentServiceOpenSequentialPortsRequest],
) (*connect.Response[devplanev1.EnvironmentServiceOpenSequentialPortsResponse], error) {
	s.t.Helper()
	assert.Equal(s.t, s.wantSequentialReq.GetEnvironmentId(), req.Msg.GetEnvironmentId())
	assertSequentialOpenRequest(s.t, s.wantSequentialReq, req.Msg)
	return connect.NewResponse(&devplanev1.EnvironmentServiceOpenSequentialPortsResponse{Ports: s.sequentialPorts}), nil
}
...
type fakeOpenNodeService struct {
	...
	wantSequentialReq *devplanev1.OpenSequentialPortsRequest
...
func (s *fakeOpenNodeService) OpenSequentialPorts(
	_ context.Context,
	req *connect.Request[devplanev1.OpenSequentialPortsRequest],
) (*connect.Response[devplanev1.OpenSequentialPortsResponse], error) {
	s.t.Helper()
	assert.Equal(s.t, s.wantSequentialReq.GetExternalNodeId(), req.Msg.GetExternalNodeId())
	assertSequentialOpenRequest(s.t, s.wantSequentialReq, req.Msg)
	return connect.NewResponse(&devplanev1.OpenSequentialPortsResponse{Ports: s.sequentialPorts}), nil
}

func sequentialPort(id string, protocol devplanev1.PortProtocol, publicPort, serverPort int32) *devplanev1.Port {
	hostname := "global.prd.ga.run.brev.nvidia.com"
	return &devplanev1.Port{
		PortId: id, Protocol: protocol, PortNumber: publicPort, ServerPort: serverPort, Hostname: &hostname,
	}
}
...
func TestOpenSequentialEnvironment(t *testing.T) {
	service := &fakeOpenEnvironmentService{
		t: t,
		wantSequentialReq: &devplanev1.EnvironmentServiceOpenSequentialPortsRequest{
			EnvironmentId: "env123", Protocol: devplanev1.PortProtocol_PORT_PROTOCOL_TCP,
			FromPortNumber: 8000, ToPortNumber: 8002, AllowedSources: []string{"10.0.0.0/8"},
		},
		sequentialPorts: []*devplanev1.Port{
			sequentialPort("port-8002", devplanev1.PortProtocol_PORT_PROTOCOL_TCP, 19002, 8002),
			sequentialPort("port-8000", devplanev1.PortProtocol_PORT_PROTOCOL_TCP, 19000, 8000),
			sequentialPort("port-8001", devplanev1.PortProtocol_PORT_PROTOCOL_TCP, 19001, 8001),
		},
	}
	_, handler := devplanev1connect.NewEnvironmentServiceHandler(service)
	newTestServer(t, handler)
	...
	cmd.SetArgs([]string{"open", "my-instance", "8000-8002", "--allow", "10.0.0.0/8"})
	var out bytes.Buffer
	cmd.SetOut(&out)
	...
	require.NoError(t, err)
	assert.Contains(t, out.String(), "Opened 3 TCP ports 8000-8002 on my-instance.")
}

func TestOpenSequentialExternalNodeJSON(t *testing.T) {
	service := &fakeOpenNodeService{
		t:    t,
		node: &devplanev1.ExternalNode{ExternalNodeId: "unode123", Name: "my-node"},
		wantSequentialReq: &devplanev1.OpenSequentialPortsRequest{
			ExternalNodeId: "unode123", Protocol: devplanev1.PortProtocol_PORT_PROTOCOL_UDP,
			FromPortNumber: 9000, ToPortNumber: 9001,
		},
		sequentialPorts: []*devplanev1.Port{
			sequentialPort("port-9000", devplanev1.PortProtocol_PORT_PROTOCOL_UDP, 20000, 9000),
			sequentialPort("port-9001", devplanev1.PortProtocol_PORT_PROTOCOL_UDP, 20001, 9001),
		},
	}
	_, handler := devplanev1connect.NewExternalNodeServiceHandler(service)
	newTestServer(t, handler)
	store := &fakeStore{user: &entity.User{ID: "user1"}, org: &entity.Organization{ID: "org1"}}
	cmd := NewCmdPorts(store)
	cmd.SetArgs([]string{"open", "my-node", "9000-9001", "--protocol", "udp", "--json"})
	var out bytes.Buffer
	cmd.SetOut(&out)
	...
	require.NoError(t, err)
	assert.JSONEq(t, `[
		{"port_id":"port-9000","endpoint":"global.prd.ga.run.brev.nvidia.com:20000","public_port":20000,"destination_port":9000,"protocol":"UDP","allowed_sources":[],"authorized_emails":[],"allow_public_unauthenticated":false,"type":"unspecified"},
		{"port_id":"port-9001","endpoint":"global.prd.ga.run.brev.nvidia.com:20001","public_port":20001,"destination_port":9001,"protocol":"UDP","allowed_sources":[],"authorized_emails":[],"allow_public_unauthenticated":false,"type":"unspecified"}
	]`, out.String())
}
...
func TestNewCmdCreatePortRejectsNonCIDRAllowedSource(t *testing.T) {
	...
	cmd.SetArgs([]string{"open", "my-instance", "2222", "--allow", "203.0.113.10"})
...
func TestParsePortRange(t *testing.T) {
	from, to, isRange, err := parsePortRange("8000-8031")
	require.NoError(t, err)
	assert.Equal(t, int32(8000), from)
	assert.Equal(t, int32(8031), to)
	assert.True(t, isRange)

	from, to, isRange, err = parsePortRange("8080")
	require.NoError(t, err)
	assert.Equal(t, int32(8080), from)
	assert.Equal(t, int32(8080), to)
	assert.False(t, isRange)

	for _, value := range []string{"8000-8000", "8001-8000", "0-2", "65534-65536", "1-2-3"} {
		t.Run(value, func(t *testing.T) {
			_, _, _, err := parsePortRange(value)
			assert.Error(t, err)
		})
	}
}

func TestOpenRangeProtocolValidation(t *testing.T) {
	for _, protocol := range []string{"http", "https", "ssh"} {
		t.Run(protocol, func(t *testing.T) {
			cmd := NewCmdPorts(&fakeStore{})
			cmd.SetArgs([]string{"open", "my-instance", "8000-8001", "--protocol", protocol})

			err := cmd.Execute()

			assert.EqualError(t, err, "port ranges are only supported for tcp and udp ports")
		})
	}
}
