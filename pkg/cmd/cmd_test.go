...
func TestExternalNodeAuth_UsesEnvAPIKey(t *testing.T) {
	...
	nodeAuth := externalNodeAuth{
		noLoginAuth:  auth.NewNoLoginAuth(fs, mockNodeAuthOAuth{}),
...
func TestExternalNodeAuth_UsesPersistedCredential(t *testing.T) {
	fs := newTestFileStore(t)
	t.Setenv(auth.APIKeyEnvVar, "")
	err := fs.SaveAuthTokens(entity.AuthTokens{AccessToken: "persisted-jwt"})
	require.NoError(t, err)
	nodeAuth := externalNodeAuth{
		noLoginAuth:  auth.NewNoLoginAuth(fs, mockNodeAuthOAuth{}),
		memLoginAuth: auth.NewLoginAuth(newEmailCachingAuthStore(fs), mockNodeAuthOAuth{}),
	...
	token, err := nodeAuth.GetAccessToken()
	require.NoError(t, err)
	assert.Equal(t, "persisted-jwt", token)
}
