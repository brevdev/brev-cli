...
func GetTextBasedONStatus(status string, t *terminal.Terminal) string {
	...
	default:
		...
		s += "\n\tYou can do that here: " + t.Yellow(fmt.Sprintf("%s/environments/new", config.GlobalConfig.GetConsoleBaseURL()))
		s += "\n\nRun " + t.Yellow("brev hello") + " to resume this walk through when your instance is ready\n"
	}
	return s
}
...
func printCompletedOnboarding(t *terminal.Terminal) {
	...
	s += "\n\nUse the console " + t.Yellow(fmt.Sprintf("(%s)", config.GlobalConfig.GetConsoleBaseURL())) + " to create a new instance or share it with people"
