...
const testAccessKey = auth.BrevAPIKeyPrefix + "test-key"

func accessKeyAddNodeFn(t *testing.T) func(*nodev1.AddNodeRequest) (*nodev1.AddNodeResponse, error) {
	t.Helper()
...
func ensureNoAccessKeyEnv(t *testing.T) {
	t.Helper()
	t.Setenv(auth.AccessKeyEnvVar, "")
}

func Test_resolveAccessKey(t *testing.T) {
	ensureNoAccessKeyEnv(t)

	if got := resolveAccessKey(); got != "" {
		t.Errorf("expected empty when no env, got %q", got)
	}

	t.Setenv(auth.AccessKeyEnvVar, auth.BrevAPIKeyPrefix+"env-key")
	if got := resolveAccessKey(); got != auth.BrevAPIKeyPrefix+"env-key" {
		t.Errorf("expected env value, got %q", got)
	}
}

func Test_runRegister_AccessKeyEnv_Fallback(t *testing.T) {
	ensureNoAccessKeyEnv(t)
	t.Setenv(auth.AccessKeyEnvVar, testAccessKey)

	regStore := &mockRegistrationStore{}
	store := testRegisterStore()
	svc := &fakeNodeService{addNodeFn: accessKeyAddNodeFn(t)}
...
func Test_runRegister_AccessKeyInvalid(t *testing.T) {
	t.Setenv(auth.AccessKeyEnvVar, "not-a-brev-key")
...
func Test_runRegister_AccessKey_SeedsSessionOnAlreadyRegistered(t *testing.T) {
	t.Setenv(auth.AccessKeyEnvVar, testAccessKey)
...
func Test_resolveOrgForAccessKey(t *testing.T) {
	tests := []struct {
		name    string
		orgs    []entity.Organization
		orgName string
		...
		{"empty", nil, "", "", "access key invalid"},
		{"multiple, name matches one", []entity.Organization{{ID: "org_1", Name: "Alpha"}, {ID: "org_2", Name: "Beta"}}, "Beta", "", "access key invalid"},
		{"multiple, no name", []entity.Organization{{ID: "org_1", Name: "Alpha"}, {ID: "org_2", Name: "Beta"}}, "", "", "access key invalid"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &mockRegisterStore{orgs: tt.orgs}
			got, err := ResolveOrgForAccessKey(s, tt.orgName)
			if tt.wantErr != "" {
				if err == nil {
...
func Test_runRegister_AccessKey_OrgMismatch(t *testing.T) {
	t.Setenv(auth.AccessKeyEnvVar, testAccessKey)
	...
	err := runRegister(context.Background(), term, store, opts, deps)
	if err == nil {
		t.Fatal("expected error when --org doesn't match the access key's org")
	}
...
func Test_runRegister_AccessKey_NoOrgFlag_UsesKeyOrg(t *testing.T) {
	t.Setenv(auth.AccessKeyEnvVar, testAccessKey)
...
func Test_runRegister_OrgMismatch(t *testing.T) {
	tests := []struct {
		name         string
		status       string
		useAccessKey bool   // set BREV_ACCESS_KEY for the new org
		useOrgFlag   bool   // pass --org for the new org
		wantWording  string // pending -> "incomplete registration"; registered -> "already registered"
	}{
		{"AccessKey_Pending", RegistrationStatusPending, true, false, "incomplete registration"},
		{"OrgFlag_Pending", RegistrationStatusPending, false, true, "incomplete registration"},
		{"AccessKey_AlreadyRegistered", RegistrationStatusRegistered, true, false, "already registered"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ensureNoAccessKeyEnv(t)
