...
ifdef env
	...
	@echo ${VERSION}
	CGO_ENABLED=0 go build -o brev-binary -ldflags "-X github.com/brevdev/brev-cli/pkg/cmd/version.Version=${VERSION}"
	@echo '#!/bin/sh' > brev
	...
	@echo 'exec "$$(cd "$$(dirname "$$0")" && pwd)/brev-binary" "$$@"' >> brev
	@chmod +x brev
else
