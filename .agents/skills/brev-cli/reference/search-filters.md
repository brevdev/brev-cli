...
#### Stoppable (`--stoppable`)
Only show instances that can be stopped and restarted without data loss.

```bash
brev search --stoppable --sort price
```

#### Rebootable (`--rebootable`)
Only show instances that can be rebooted.

```bash
brev search --rebootable --sort price
```

#### Flex Ports (`--flex-ports`)
Only show instances with configurable firewall/port rules.

```bash
brev search --flex-ports --sort price
```
